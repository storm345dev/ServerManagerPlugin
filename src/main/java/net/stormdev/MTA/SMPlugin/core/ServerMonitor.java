...
public class ServerMonitor implements Runnable {
	...
	public static boolean overloadPrevention(){
		...
		boolean dynClose = Core.plugin.getServerShouldOpenCloseDynamically();
		
		if(freeMemory < 150){
			System.gc();
			freeMemory = (long) (Runtime.getRuntime().freeMemory() * 0.00097560975 * 0.00097560975); //In MB
			if(freeMemory < 150 && dynClose){ //If, after gc, the memory is still running out
				//Enable and disable AI
				Core.plugin.setServerOpen(false);
				return true;
			}
		}
		else if(dynClose) {
			//re-enable AI if disabled
			if(!Core.plugin.isServerOpen()){
				Core.plugin.setServerOpen(true);;
			}
		}
		return false;
	}
	...
	public void start(){
		...
		LAST_TICK = 0L;
		running = false;
		//Core.plugin.lagReducer.cancel();
		Core.plugin.serverMonitor = Bukkit.getScheduler().runTaskTimer(Core.plugin,
	...
	public void restart(){
		...
		running = false;
		failedSyncTasks = 0;
		Core.plugin.serverMonitor.cancel();
		Core.plugin.serverMonitor = Bukkit.getScheduler().runTaskTimer(Core.plugin,
				new ServerMonitor(), 100L, 1L);
		return;
	}
